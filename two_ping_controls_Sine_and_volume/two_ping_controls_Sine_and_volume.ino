//2 ping sensors.  one controls amplitude, the other a sine wave within a specified frequency.  Written by Darcy Neal www.ladybrainstudios.com
//
...
#define PING_INTERVAL 29 //milliseconds between sensor pings (29ms is about the mid to avoid cross-sensor echo).
...
const unsigned int PING_CHECK_INTERVAL = SECOND / 20;  //how often the ultrasonic sensor is checked
        ...
        if (currentTime >= nextTime) { //will always be true the first time...Timer incrememnter
                ...
                if (whichPing ==1) {
                        ...
                        if (rawPing1 == 0 || rawPing1 > 25) { //if sensor is outside the detection range
                                raw_interactive_vol = MAX_RAW_VOLUME / 2; // was / 2
                        }
                        else {
                ...
                //get values from ping 2
                        ...
                        if (rawPing2 >= 1 && rawPing2 <= 25) { //if sensor is triggered within the detection range
                                rawSine2 =  map(rawPing2, 0, 25, 50, 400);//map it to the frequency range desired...  could this be done on a logarithmic curve??
